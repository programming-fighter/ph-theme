...
const ProductCardOne = ({ item, store_id }: any) => {
    ...
    const campPrice = (getPrice(productGetPrice, (camp?.discount_amount), camp?.discount_type))
    ...
    const filterOfferProduct = (item: any) => {
        ...
        // })
        axios.post("https://admin.ebitans.com/api/v1/" + 'get/offer/product', productDetails)
            .then((res: any) => {

                if (!res?.error) {

                    let itemRegularPrice = (getPrice(item?.regular_price, item?.discount_price, item?.discount_type))
                    let campaignPrice = (getPrice(itemRegularPrice, (res?.discount_amount), res?.discount_type))
            ...
            <div className="drop-shadow-xl w-full relative">
                ...
                <div className="card-body p-4 bg-white">
                    ...
                    <h2 className="tracking-widest font-normal text-sm text-gray-600">
                        <div dangerouslySetInnerHTML={{ __html: item?.description?.slice(0, 18) }} className='apiHtml'></div>
                    </h2>

                    <h6 className='text-lg font-medium flex items-center'>
                        <Taka tk={camp?.status === "active" ? campPrice : productGetPrice} />
                    ...
                    {result?.qty ? <div
                        ...
                        <div onClick={() => add_to_cart(item)} className="absolute bottom-6 right-2">
                            <HoverIcon text={"Add to Cart"} >
                                <BsBagPlus className='h-5 w-5 text-2xl font-serif font-bold' />
                            </HoverIcon>
                        </div> : <Link href={'/product/' + item?.id + '/' + item?.slug} className="absolute bottom-6 right-2">
